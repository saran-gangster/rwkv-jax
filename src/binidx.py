...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        prefix = os.path.join(tmpdir, "test_dataset")
        
        # Write dummy data
        with MMapIndexedDataset.Index.writer(index_file_path(prefix), np.int32) as index_writer:
            with open(data_file_path(prefix), 'wb') as data_file:
                dummy_data = [
                ...
                for i, arr in enumerate(dummy_data):
                    data_file.write(arr.tobytes())
                    sizes.append(len(arr))
                    if i > 0:
                        doc_idx.append(doc_idx[-1] + sizes[-2])
                index_writer.write(sizes, doc_idx)
